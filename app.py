...
def get_multimodal_prompt(summary_type="detailed"):
    ...
    base_instructions = """
You are an expert AI tutor and note-taking assistant. Your task is to transform multimodal lecture data (audio transcript, visual text from slides/board, and subtitles) into clear, structured notes suitable for studying.

Instructions:
1. Generate structured lecture notes that combine information from:
   - Audio transcript (primary source for explanations and spoken content)
   - Visual text (prioritize for formulas, definitions, and written content from slides/board)
   - Subtitles (use only as fallback if content is missing)

2. Prioritization rules:
   - Use visual text for formulas, equations, definitions, and written content
   - Use audio transcript for explanations, concepts, and spoken narrative
   - Use subtitles only if audio or visual text is unclear or missing

3. Organization:
   - Organize content into clear headings and subheadings
   - Use bullet points for clarity and emphasis
   - Highlight key formulas, terms, or definitions using bold or clear markers

4. Format:
   - Use Markdown formatting
   - Include timestamps for major topic transitions if helpful
   - Structure with proper hierarchy (H1, H2, H3, bullets)
"""
    
    if summary_type == "short":
        summary_instructions = """
5. Summary Type: SHORT SUMMARY
   - Create a concise, high-level overview
   - Focus on main topics, key concepts, and essential formulas only
   - Skip detailed explanations and examples
   - Aim for 20-30% of the original content length
   - Each major topic should be 2-3 bullet points maximum
   - Prioritize formulas, definitions, and key takeaways
"""
    else:  # detailed
        summary_instructions = """
5. Summary Type: DETAILED SUMMARY
   - Create comprehensive, in-depth notes
   - Preserve ALL important points, concepts, formulas, and definitions
   - Include examples or explanations if they clarify a concept
   - Remove redundancy but keep completeness
   - Use simple, student-friendly language
   - If multiple topics exist, separate them clearly
   - Ensure notes are readable as a standalone document
   - Aim for 60-80% of the original content length
"""
...
def generate_multimodal_notes(fused_chunks, summary_type="detailed"):
    ...
    Args:
        fused_chunks: List of fused multimodal data chunks
        summary_type: "short" for concise summary or "detailed" for comprehensive notes
    """
    # Get appropriate prompt based on summary type
    prompt = get_multimodal_prompt(summary_type)
    
    # Format fused chunks for LLM
    formatted_data = ""
    ...
    response = model.generate_content(prompt + formatted_data)
...
def answer_question_with_context(question, relevant_chunks, youtube_url=None):
    ...
    for i, result in enumerate(relevant_chunks):
        ...
        text = chunk["combined_text"][:500]  # Limit length
        context_parts.append(f"[{time_str}] {text}")
    
    context = "\n\n".join(context_parts)
    
    # Create prompt with hallucination guard
...
Answer (cite timestamps like [MM:SS]):"""
    ...
    if youtube_url:
        ...
        if video_id:
            ...
            # Find timestamps in format [MM:SS] or [HH:MM:SS]
            answer = re.sub(r'\[(\d{1,2}:\d{2}(?::\d{2})?)\]', make_timestamp_clickable, answer)
